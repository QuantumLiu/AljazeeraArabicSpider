...
class category():
    def __init__(self,name,url):
        ...
        self.lms=[self.lm_root.format(n=n+1) for n in range(self.max_page)]
        self.articels=[]
        self.log=''
    def cur_no(self,lm):
    ...
    def process(self,lm):
        ...
        res=requests.get(lm,headers=self.ua)
        text=res.text
        ...
        articels=[{'title':t,'abstract':ab,'url':u,'img':i} for t,ab,u,i in zip(titles,abstracts,urls,imgs)]
        if not articels:
            print('Got empty during crawling page {p}\nHTML text : {h} ,return []'.format(p=lm,h=text))
        else:
...
def get_params(cat_res):
    ...
    params={p:re.findall(r_root.format(param=p),cat_res.text)[0].strip().replace('#','') for p in p_l}
